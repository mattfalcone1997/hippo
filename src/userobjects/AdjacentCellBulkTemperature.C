...
AdjacentCellBulkTemperature::AdjacentCellBulkTemperature(const InputParameters & params)
  : GeneralUserObject(params), HippoInterface(this), _T_bulk{getTBoundaryField()}
{
}
...
{
  const auto & T{getFvMesh().lookupObject<Foam::volScalarField>("T")};
  const Foam::label & boundaryId{
      getFvMesh().boundary().findIndex(getParam<SubdomainName>("boundary"))};
...
AdjacentCellBulkTemperature::spatialValue(const Point & point) const
{
  ...
  const Foam::fvPatch & patch = getFvMesh().boundary()[boundaryId];
